...
const storedWealth=storage.get(keys.wealth,{});
let wealth={...defaults,...(storedWealth&&typeof storedWealth==='object'&&!Array.isArray(storedWealth)?storedWealth:{})};
...
function inject(){
 ...
 if(!q('#wealth')){
  ...
  bindNav();
  document.querySelectorAll('[data-wealth]').forEach(el=>el.addEventListener('input',()=>{wealth[el.dataset.wealth]=Number(el.value||0);storage.set(keys.wealth,wealth);renderPro()}));
 }
}
...
function renderPro(){
 ...
 if(typeof homePlan!=='undefined'&&homePlan){homePlan.equity=wealth.cash+wealth.etf+wealth.stocks+wealth.crypto+wealth.other;homePlan.pillar3aBalance=wealth.pillar3a;storage.set(keys.homePlan,homePlan);if(typeof renderHome==='function')renderHome();}
}
